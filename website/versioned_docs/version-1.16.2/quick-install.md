...
OpenEverest Helm charts can be found in the [openeverest/helm-charts](https://github.com/openeverest/helm-charts/tree/v1.x/charts/everest) repository on GitHub.
...
1. Install [Helm v3](https://docs.helm.sh/using_helm/#installing-helm).
...
3. Set up a Kubernetes cluster.
   ...
   [Create EKS cluster →](install/eks.md)  [Create GKE cluster →](install/gke.md)

4. Verify that you have access to the Kubernetes cluster that you want to use with OpenEverest. By default, OpenEverest uses the kubeconfig file available under `~/.kube/config`.
...
### Install OpenEverest and access it using Ingress

**Prerequisite**

- An Ingress controller (for example, Nginx) installed on your Kubernetes cluster
- If TLS is required on your Ingress endpoint, a **Secret** containing the TLS certificates

**Example**

To install OpenEverest and access using [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/#what-is-ingress), follow these steps:
...
1. Retrieve the `admin` password.
   ...
   :::note
   The default admin password is stored in plain text. It is highly recommended to update the password using `everestctl` to ensure the passwords are hashed.
   :::
...
The `kubectl port-forward` command in Kubernetes is used to create a temporary connection between your local machine and a specific Kubernetes resource (for example, a Pod, Service, or Deployment) by forwarding traffic from a local port to a port on the resource.

1. Run the following command to set up a port-forward to the OpenEverest server service:
...
{/*
[Monitor the database with PMM →](../use/monitorDBs.md)
*/}
